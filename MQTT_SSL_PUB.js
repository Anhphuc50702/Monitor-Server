...
const mqtt=require('mqtt');
        var customer = mqtt.connect('mqtt://broker.emqx.io:1883',{rejectUnauthorized : false,clientId: "quectel-device-x509self1"});//, username: "quectel-hub.azure-devices.net/quectel-device-x509self1/?api-version=2018-06-30", ca:caFile, key:KEY,cert:CERT});
        customer.on('connect',function(){	
        customer.publish("testtopic/1", "abc");
        console.log("Client has published");
        });
